...
$photo = "photo/".$names.".".$tipe;

$file = basename($_FILES["ktp"]["name"]);
$tipe = strtolower(pathinfo($file,PATHINFO_EXTENSION));
...
if ($check2 == true){
    move_uploaded_file($_FILES["sim"]["tmp_name"], $license);
    $check3 = true;
}else{
    $check3 = false;
}
...
if ($cekMail->num_rows != 0) {
    ...
    if  ($check == true && $check2 == true && $check == true){
        $add = $db->query("INSERT INTO drivers VALUES(NULL, '$nama', '$email', '$pass', '$notelp', '$domisili', '$photo', '$plate', '$license', '$ktp' , '0', '$str3', '0')");
    	if ($add) {
    ...
    // 		$mail->SMTPDebug = 2;
    		...
    		echo "<script>location.href='add-driver.php'</script>";
    	}
    }else{
        $msg = "Something went wrong, please try again!";
	    echo "<script>alert('".$msg."')</script>";
